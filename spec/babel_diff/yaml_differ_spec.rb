...
describe BabelDiff::YamlDiffer do
  ...
  let(:previous_version) { File.read("spec/test_files/seed.yml") }
  subject(:differ) { BabelDiff::YamlDiffer.new(current_version, previous_version) }

  describe '#updates' do
    let(:expected_updates) { File.read("spec/test_files/expected_updates.yml")}
  ...
  describe '#additions' do
    let(:expected_additions) { File.read("spec/test_files/expected_additions.yml") }
